...
flatten_para <- function(x, ...) {
  ...
  blocks[needs_p] <- p(trim(blocks[needs_p]))
...
flatten_text <- function(x, ...) {
  ...
  md <- purrr::map_chr(x, as_markdown, ...)
  paste(md, collapse = "")
}

select_from_id <- function(x, id) {
  target <- sapply(x, function(x) x$id %in% id)
  x[target][[1]]
}
...
order_rdfile <- function(
  ...
  keep_first <- keep_first[keep_first %in% names(x)]
  keep_last <- keep_last[keep_last %in% names(x)]
  x <- x[!(names(x) %in% remove)]
  x <- c(
    x[keep_first],
    x[!(names(x) %in% c(keep_first, keep_last))],
    x[keep_last]
  )
}
...
as_markdown.rdfile <- function(x, ...) {
  ...
  x <- order_rdfile(
    x,
    keep_first = c("usage", "arguments"),
    remove = c("name", "description", "alias"),
    keep_last = c("value", "see_also")
  )

  rest <- paste0(sapply(x, as_markdown), collapse = "")
...
#' @export
as_markdown.rdsection <- function(x, ...) {
  sprintf("## %s\n\n%s\n\n", x$title, x$content)
}

#' @export
as_markdown.LIST <-  flatten_text

# Leaves  -----------------------------------------------------------------

#' @export
as_markdown.character <- function(x, ...) {
  as.character(x)
}
#' @export
...
#' @export
as_markdown.tag_method <- function(x, ...) method_usage(x, "S3")
#' @export
as_markdown.tag_S3method <- function(x, ...) method_usage(x, "S3")
#' @export
as_markdown.tag_S4method <- function(x, ...) method_usage(x, "S4")
...
as_markdown.tag_ifelse <- function(x, ...) {
  if (x[[1]] == "html") as_markdown(x[[2]], ...) else as_markdown(x[[3]], ...)
}
...
as_markdown.tag_tabular <- function(x, ...) {
  align_abbr <- strsplit(as_markdown(x[[1]], ...), "")[[1]]
  ...
  cell_contents <- paste0("<td>", trim(cell_contents), "</td>")
...
#' @export
as_markdown.tag_emph <-         tag_wrapper("<em>", "</em>")
#' @export
as_markdown.tag_strong <-       tag_wrapper("<strong>", "</strong>")
#' @export
as_markdown.tag_bold <-         tag_wrapper("<b>", "</b>")

#' @export
as_markdown.tag_dQuote <-       tag_wrapper("&#8220;", "&#8221;")
#' @export
as_markdown.tag_sQuote <-       tag_wrapper("&#8216;", "&#8217;")

#' @export
as_markdown.tag_code <-         function(x, ..., auto_link = TRUE) {
  text <- flatten_text(x, ...)
...
#' @export
as_markdown.tag_preformatted <- function(x, ...) {
...
#' @export
as_markdown.tag_kbd <-          tag_wrapper("<kbd>", "</kbd>")
#' @export
as_markdown.tag_samp <-         tag_wrapper('<samp>',"</samp>")
#' @export
as_markdown.tag_verb <-         tag_wrapper("<code>", "</code>")

#' @export
as_markdown.tag_pkg <-          tag_wrapper('<span class="pkg">',"</span>")
#' @export
as_markdown.tag_file <-         tag_wrapper('<code class="file">', '</code>')

#' @export
as_markdown.tag_var <-          tag_wrapper("<var>", "</var>")
#' @export
as_markdown.tag_env <-          tag_wrapper('<code class="env">', '</code>')
#' @export
as_markdown.tag_option <-       tag_wrapper('<span class="option">',"</span>")
#' @export
as_markdown.tag_command <-      tag_wrapper("<code class='command'>", "</code>")

#' @export
as_markdown.tag_dfn <-          tag_wrapper("<dfn>", "</dfn>")
#' @export
as_markdown.tag_cite <-         tag_wrapper("<cite>", "</cite>")
#' @export
as_markdown.tag_acronym <-      tag_wrapper('<acronym>','</acronym>')

#' @export
as_markdown.tag_out <- function(x, ...) flatten_text(x, ..., escape = FALSE)

# Insertions --------------------------------------------------------------

#' @export
as_markdown.tag_R <-     function(x, ...) '<span style="R">R</span>'
#' @export
as_markdown.tag_dots <-  function(x, ...) "..."
#' @export
as_markdown.tag_ldots <- function(x, ...) "..."
#' @export
